      ...
      <div style={{ padding: '24px' }}>
        <Tabs activeKey={tab} onChange={(activeKey) => setTab(activeKey)}>
          ...
          <Tabs.TabPane key="2" tab="用户注册">
            <UserForm onAddUser={addUser} onCancel={() => setIsEditing(false)} />
          </Tabs.TabPane>
        </Tabs>
      </div>
      <Modal title="信息编辑" open={isEditing} onCancel={() => setIsEditing(false)} footer={null}>
        <UserForm isEditing selectedUser={selectedUser} onUpdateUser={updateUser} onCancel={() => setIsEditing(false)} />
      </Modal>
    </>
  );
}
